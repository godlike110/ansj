...
public class NewsAnalysis4 {

	static List<String> docs = new ArrayList<String>();
	static Map<String, Integer> map = new TreeMap();
	static String result = new String();
	
	static String fileSpread;
	
	static {
		fileSpread = File.separator;
	}
		...
		try {
			...
			if (!file.isDirectory()) {
				...
				for (int i = 0; i < filelist.length; i++) {
					...
					File readfile = new File(filePath + fileSpread + filelist[i]);
