...
import cn.focus.estatedic.thread.EstateThread;
import cn.focus.estatedic.thread.ProcessThread;
...
public class Demo {
	...
	public static int m1 = AppConstants.m1 ; //输入参数，每一组训练文本集合包含的房产新闻数目
	public static int m2 = AppConstants.m2;  //输入参数，每一组训练文本集合包含的非房产新闻数目
	public static int k1 = AppConstants.k1; // 根据总的房产新闻数目和m1计算得到k1，房产新闻总的组数
	public static int k2 = AppConstants.k2; // 根据总的非房产新闻数目和m2计算得到k2，非房产新闻总的组数
	public static int k1min = 0;
	public static int k2min = 0;
	
	public static int wordsPerLda = 100; //每一轮主题模型LDA算法挑选出来的单词数目
	public static double ratio = 0; //筛选单词的控制变量
	static int weight = 10; // 新词nw和用户自定义user define的权重
	...
	public Demo(int m1, int m2, int wordsPerLda, double ratio, int weight){
		this.m1 = m1;
		this.m2 = m2;
	...
	public void process() throws InterruptedException {
		...
		CountDownLatch cdl = new CountDownLatch(k1-k1min);
		for (int i = k1min; i < k1; i++) {
			
			EstateThread et = new EstateThread(cdl,i,k2min,k2,estateFilePath,m1,nonEstateFilePath,m2,weight);
			et.start();
		}
		cdl.await();
	...
	*/
	 ...
	 */
	public static List<WordProbability> calLdaBeta(String resultFileName, int i, int j) {
	 ...
	 * @return 
	 */
	public static Map<String, WordProbability> findWordsByCount(int gi, int gj,
	...
	public static Map<String, WordProbability> findWordsByProbability(int gi, int gj,
			List<List<WordProbability>> list) {
Map<String, WordProbability> m = new HashMap();
	...
	public void mergeWordsByProbability(){ 
		...
		List<Map.Entry<String,BigDecimal>> mappingList = null; 
		  ...
		  for(Map.Entry<String,BigDecimal> mapping:mappingList){ 
		   ...
		   BufferedWriter writer = null;
			String resultFileName = AppConstants.PATH + "DemoResult/final";
